...
//}
    ...
    // 회원가입
    @PostMapping("/signup")
    public ResponseEntity<ResponseHandler> register(@RequestBody UserFormDTO dto){
        return null;
    }
    ...
    public ResponseEntity<ResponseHandler<Boolean>> checkNicknameDuplicate(@RequestBody String nickname){
        ...
        ResponseHandler<Boolean> response = new ResponseHandler<>(
                isDuplicated,
                isDuplicated ? HttpStatus.CONFLICT.value() : HttpStatus.OK.value(),
...
//    void logout();
    ...
    // 에러 처리 필요
    @GetMapping("/{userid}")
    public ResponseEntity<ResponseHandler> getUser(@PathVariable Long userId){
        try {
...
//        return null;
    ...
    // 회원 정보 수정
    @PutMapping("/{userid}")
    public ResponseEntity<ResponseHandler> updateUser(@PathVariable Long userid, @RequestBody UserFormDTO dto){
        return null;
    }
}
